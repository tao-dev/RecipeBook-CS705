/*
 This class is to handle PeanutButterCookie 
...
namespace Amanda_Recipe_Book
{
    class PeanutButterCookie
    {
        ...
        {
            FILEPATH.Add(IDD.PATHSIDD.First(p => p.Contains("1 ing")));//it is getting file path of 3 ing file i.e 3 ingredient.
            // actually PATHSIDD contains all files path so it is getting of specified file name only.
            FILEPATH.Add(IDD.PATHSIDD.First(p => p.Contains("1 ins")));
        }
        #endregion
        ...
        private void saveInXML()
        {
            ...
            doc.LoadXml("<item><name>Peanut Butter Cookie</name></item>");

            foreach (var item in PBCDATA)
            {
                // Add a price element.
                XmlElement newElem = doc.CreateElement("item1");
